...
contract ChainlinkOracle is Ownable2Step, Pausable {
    ...
    function getPriceIn18Decimals(address token) external view whenNotPaused returns (uint256) {
        (int256 _answer, uint8 _decimals) = getPrice(token);
    ...
    function _badPriceDeviation(
        ...
        if (currentResponse.answer > prevResponse.answer) {
            ...
            _deviation =
                uint256(prevResponse.answer - currentResponse.answer) * SAFE_MULTIPLIER / uint256(prevResponse.answer);
        }
