...
contract USwap is Ownable, ISwap {
  ...
  */
    function swapExactTokensForTokens(ISwap.SwapParams memory sp) external override returns (uint256 amountOut) {
        ...
        if(sp.amountOutMin == 0) revert Errors.EmptyDepositAmount();
        ...
        IERC20(sp.tokenIn).safeApprove(address(router), sp.amountIn);

        uint256[] memory amounts = router.swapExactTokensForTokens(sp.amountIn, sp.amountOutMin, sp.tokenIn, sp.tokenOut, sp.deadline);
