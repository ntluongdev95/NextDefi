...
contract ChainlinkOracle is Ownable2Step, Pausable {
  ...
  /**
   ...
   function _getChainlinkResponse(address feed) internal view returns (ChainlinkResponse memory) {
    (uint80 roundId, int256 answer, uint256 timestamp, , ) = AggregatorV3Interface(feed).latestRoundData();
    uint8 decimals = AggregatorV3Interface(feed).decimals();
    return ChainlinkResponse(roundId, answer, timestamp, true, decimals);
}